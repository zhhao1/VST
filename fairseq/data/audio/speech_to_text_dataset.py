...
class SpeechToTextDataset(FairseqDataset):
    ...
    def __init__(
        ...
        assert utt_paths is None or len(utt_paths) == self.n_samples
        ...
        self.word_dropout = cfg.word_dropout
        self.word_droprate = cfg.word_droprate
        self.pretrain_utt = cfg.pretrain_utt
    ...
    def __getitem__(self, index: int) -> SpeechToTextDatasetItem:
        ...
        if self.pretrain_utt:
            utt_rep = get_features_or_waveform(
            self.utt_paths[index],
        )
            utt_rep = torch.from_numpy(utt_rep).unsqueeze(0).float()
        ...
        return SpeechToTextDatasetItem(
            index=index, source=source, target=target, speaker_id=speaker_id, utt_rep=utt_rep
        )         
    
    def random_drop(self, text):
        if self.word_dropout and self.is_train_split:
            nwords = len(text)
            new_text = torch.IntTensor(nwords)
            unk = self.tgt_dict.unk()
            eos = self.tgt_dict.eos()           
            word_droprate = self.word_droprate
            probs = torch.empty(nwords-1).uniform_(0,1)
            new_text[:-1] = torch.where(probs > word_droprate, text[:-1], unk)
            new_text[-1] = eos
            return new_text
        else:
            return text
    ...
    def collater(
        ...
        if self.tgt_texts is not None:
            ...
            prev_output_tokens = fairseq_data_utils.collate_tokens(
                [self.random_drop(x.target) for x in samples],
                self.tgt_dict.pad(),
                self.tgt_dict.eos(),
                left_pad=False,
        ...
        utt_reps = None
        if self.pretrain_utt:
            utt_reps = torch.cat([x.utt_rep for x in samples], dim=0)
            utt_reps = utt_reps.index_select(0, order)
        ...
        out = {
            ...
            "net_input": net_input,
            "speaker": speaker,
            "target": target,  #B*T, [token, token, eos, pad]
            "target_lengths": target_lengths,
            "ntokens": ntokens,
            "nsentences": len(samples),
        }
        #logging.info(out['net_input']['src_tokens'].size())
        if return_order:
            out["order"] = order
        return out
...
class SpeechToTextDatasetCreator(object):
    ...
    KEY_UTT = "utt_pre"
    # default values
    DEFAULT_SPEAKER = DEFAULT_SRC_TEXT = DEFAULT_LANG = DEFAULT_UTT = ""
    ...
    def _from_list(
        ...
        utt_paths = [(audio_root / s.get(cls.KEY_UTT, cls.DEFAULT_UTT)).as_posix() for s in samples]
