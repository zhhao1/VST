    ...
    def __init__(self, root: str, lang: str, split: str) -> None:
        ...
        for wav_filename, _seg_group in groupby(segments, lambda x: x["wav"]):
            ...
            for i, segment in enumerate(seg_group):
                ...
                n_frames = int(float(segment["duration"]) * sample_rate)  #一条音频的采样点数目，下面的是一条音频提取出的mfcc特征的数目
...
def process(args):
    root = Path(args.data_root).absolute() #跨平台处理路径，并获取绝对路径
    for lang in MUSTC.LANGUAGES:
        cur_root = root / f"en-{lang}"
        if not cur_root.is_dir():
            print(f"{cur_root.as_posix()} does not exist. Skipped.") #as_posix,路径分割符，处理windows格式到unix格式
            continue
        # Extract features
        ...
        for split in MUSTC.SPLITS:
            ...
            if args.use_audio_input:
                ...
                for waveform, sample_rate, _, _, _, utt_id in tqdm(dataset):
                    ...
                    #print(_wavform.numpy().squeeze(axis=0).shape)
                    #here need to do _wavform.numpy().squeeze(axis=0) 
                    sf.write(
        ...
        for split in MUSTC.SPLITS:
            ...
            df = filter_manifest_df(df, is_train_split=is_train_split) #过滤训练集
